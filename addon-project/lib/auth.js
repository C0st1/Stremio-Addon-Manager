...
/**
 * Creates a signed session token (HMAC-SHA256 JWT-like structure).
 *
 * @param {object} payload            Arbitrary data to embed (must include `authKey`).
 * @param {number} [maxAgeSeconds=1800] Token lifetime in seconds (default: 30 minutes).
 * @returns {string} Signed token string.
 */
function createSessionToken(payload, maxAgeSeconds = 1800) {
 ...
 *
 * Accepts an optional pre-verified payload to avoid re-parsing the cookie
 * when the token has already been verified in the calling handler.
 *
 * @param {import('http').IncomingMessage}  req
 * @param {import('http').ServerResponse}   res
 * @param {object|null} [verifiedPayload=null] Pre-verified session payload.
 */
function refreshSession(req, res, verifiedPayload = null) {
  ...
  // Refresh the cookie if less than 15 minutes remain
  if (remaining < 900) {
